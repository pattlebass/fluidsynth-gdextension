...
opts.Add(
    "source_dirs", "List of source directories (comma-separated)", "src"
)  # Directory for source files
opts.Add(
    "source_exts",
    "List of source file extensions (comma-separated)",
    ".cpp,.c,.cc,.cxx",
)
opts.Add(
    "include_dirs", "List of include directories (comma-separated)", "src"
)  # Directory for headers - some might want to create a separate include directory
opts.Add("doc_output_dir", "Directory for documentation output", "gen")
opts.Add(
    "precision", "Floating-point precision (single or double)", "single"
)  # Default to single
opts.Add(
    "bundle_id_prefix",
    "Bundle identifier prefix (reverse-DNS format)",
    "com.gdextension",
)  # Default prefix
opts.Add(
    EnumVariable(
...
bundle_id_prefix = env.get(
    "bundle_id_prefix", "com.gdextension"
)  # Ensure prefix defaults to com.gdextension
...
if env.get("target") in ["editor", "template_debug"]:
    try:
        ...
        doc_data = env.GodotCPPDocData(
            doc_output_file, source=Glob("doc_classes/*.xml")
        )
...
threads_suffix = (
    ".threads" if env["platform"] == "web" and threads_val in ("yes", "true") else ""
)
...
if env["platform"] in ["macos", "ios"]:
    ...
    temp_lib = env.SharedLibrary(
        f"bin/{env['platform']}/{lib_filename}", source=sources
    )
    if env["platform"] == "macos":
        ...
        env.Command(
            ...
            lambda target, source, env: write_info_plist(
                ...
                source,
                env,
                generate_info_plist("macos", env["target"], precision),
            ),
        )
        ...
        env.Depends(
            library, plist_file
        )  # Ensure Info.plist is created before the framework binary
        ...
        env.Command(
            ...
            lambda target, source, env: write_info_plist(
                ...
                source,
                env,
                generate_info_plist("ios", env["target"], precision),
            ),
        )
        ...
        temp_framework = env.Command(
            ...
            temp_lib,
            [f"mkdir -p {temp_framework_dir}", f"cp $SOURCE $TARGET"],
        )
        env.Depends(
            temp_framework, plist_file
        )  # Ensure Info.plist is created before the framework binary
    ...
    # Android requires lib prefix on .so files
    android_lib_filename = f"lib{libname}{suffix}{env.subst('$SHLIBSUFFIX')}"
    library = env.SharedLibrary(
        f"bin/{env['platform']}/{android_lib_filename}", source=sources
    )
    ...
    install_source = [t for t in library if not str(t).lower().endswith(".a")]
